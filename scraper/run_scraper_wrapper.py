...
def main():
    ...
    scraper_path = os.path.join(script_dir, "linkedin_scraper.py")

    spec = importlib.util.spec_from_file_location("local_linkedin_scraper", scraper_path)
    if not spec or not spec.loader:
        raise ModuleNotFoundError(
            "Could not load scraper module from "
            f"'{scraper_path}'."
        )
