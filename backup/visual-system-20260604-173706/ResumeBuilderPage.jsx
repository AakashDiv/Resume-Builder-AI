...
export default function ResumeBuilderPage() {
  ...
  return (
    <div
      className={`premium-builder grid gap-5 print:block ${
        activeStep === "finalize"
          ? isSidebarExpanded
            ? "xl:grid-cols-[260px,minmax(0,1fr)]"
            : "xl:grid-cols-[88px,minmax(0,1fr)]"
          : isSidebarExpanded
            ? "xl:grid-cols-[260px,minmax(0,1fr),minmax(380px,460px)]"
            : "xl:grid-cols-[88px,minmax(0,1fr),minmax(380px,460px)]"
      } min-h-screen text-slate-900`}
    >
      <aside className="premium-sidebar print:hidden">
        <div>
          <div className="mb-3 flex items-center justify-between gap-2">
            {isSidebarExpanded ? (
              <div>
                <p className="text-xs font-black uppercase text-slate-500">Resume Studio</p>
          ...
          {isSidebarExpanded ? (
            <p className="mt-2 text-xs text-slate-500">
              {Object.values(completedMap).filter(Boolean).length} of {steps.length - 1} core sections ready
            </p>
          ) : null}
        </div>
                ...
                {isSidebarExpanded ? (
                  <>
                    <span className="flex min-w-0 items-center gap-2">
                      <span className="premium-step-icon"><StepIcon className="shrink-0 text-sm" aria-hidden /></span>
                      <span className="min-w-0">
                        <span className="block truncate">{step.label}</span>
                        <span className="block text-[11px] font-semibold text-slate-400">{done ? "Complete" : active ? "In progress" : "Pending"}</span>
                      </span>
                    </span>
        ...
        {isSidebarExpanded ? (
          <div className="premium-ats-card mt-5">
            <p className="text-xs font-black uppercase text-slate-500">ATS Pulse</p>
            <div className="mt-3 flex items-end justify-between">
              <span className="text-3xl font-black text-slate-950">{qualityReport.score}</span>
              <span className="text-xs font-bold text-slate-500">/100</span>
            </div>
            <div className="mt-3 h-2 rounded-full bg-slate-100">
              <div className="h-2 rounded-full bg-emerald-500" style={{ width: `${Math.min(100, qualityReport.score || 0)}%` }} />
            </div>
            <p className="mt-3 text-xs leading-5 text-slate-500">
              {missingChecks.length ? `${missingChecks.length} improvement suggestion${missingChecks.length > 1 ? "s" : ""} remaining.` : "Resume is looking strong."}
            </p>
          </div>
        ) : null}
      </aside>
      ...
      <section className="premium-editor-panel print:hidden">
        <header className="premium-action-bar">
          <div>
            <p className="text-xs font-black uppercase text-brand-700">Section {activeStepIndex + 1} of {steps.length}</p>
          ...
          <div className="premium-toolbar">
            ...
            <button onClick={resetDraft} className="premium-secondary-button">
              Reset Draft
            </button>
            <button onClick={handleSaveDraft} className="premium-secondary-button">
              Save Draft
            </button>
            <button
        ...
        <div className="premium-engagement-grid mb-5">
          <div>
